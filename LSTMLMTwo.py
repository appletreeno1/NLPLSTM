...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.W_ii = nn.Linear(256, 2*n_hidden, bias= False)
        self.b_ii =nn.Parameter(torch.ones([2*n_hidden]))
        self.W_hi = nn.Linear(256, 2*n_hidden, bias = False)
        self.b_hi =nn.Parameter(torch.ones([2*n_hidden])) 

        self.W_if = nn.Linear(256, 2*n_hidden, bias= False)
        self.b_if =nn.Parameter(torch.ones([2*n_hidden]))
        self.W_hf = nn.Linear(256, 2*n_hidden, bias = False)
        self.b_hf =nn.Parameter(torch.ones([2*n_hidden])) 

        self.W_ig = nn.Linear(256, 2*n_hidden, bias= False)
        self.b_ig =nn.Parameter(torch.ones([2*n_hidden]))
        self.W_hg = nn.Linear(256, 2*n_hidden, bias = False)
        self.b_hg =nn.Parameter(torch.ones([2*n_hidden])) 

        self.W_io = nn.Linear(256, 2*n_hidden, bias= False)
        self.b_io =nn.Parameter(torch.ones([2*n_hidden]))
        self.W_ho = nn.Linear(256, 2*n_hidden, bias = False)
        self.b_ho =nn.Parameter(torch.ones([2*n_hidden])) 

        self.W_ii1 = nn.Linear(256, 2*n_hidden, bias= False)
        self.b_ii1 =nn.Parameter(torch.ones([2*n_hidden]))
        self.W_hi1 = nn.Linear(256, 2*n_hidden, bias = False)
        self.b_hi1 =nn.Parameter(torch.ones([2*n_hidden])) 

        self.W_if1 = nn.Linear(256, 2*n_hidden, bias= False)
        self.b_if1 =nn.Parameter(torch.ones([2*n_hidden]))
        self.W_hf1 = nn.Linear(256, 2*n_hidden, bias = False)
        self.b_hf1 =nn.Parameter(torch.ones([2*n_hidden])) 

        self.W_ig1 = nn.Linear(256, 2*n_hidden, bias= False)
        self.b_ig1 =nn.Parameter(torch.ones([2*n_hidden]))
        self.W_hg1 = nn.Linear(256, 2*n_hidden, bias = False)
        self.b_hg1 =nn.Parameter(torch.ones([2*n_hidden])) 

        self.W_io1 = nn.Linear(256, 2*n_hidden, bias= False)
        self.b_io1 =nn.Parameter(torch.ones([2*n_hidden]))
        self.W_ho1 = nn.Linear(256, 2*n_hidden, bias = False)
        self.b_ho1 =nn.Parameter(torch.ones([2*n_hidden])) 
        # self.LSTM = nn.LSTM(input_size=emb_size, hidden_size=n_hidden)
    ...
    def forward(self, X):
        ...
        c_t_1 = torch.randn((n_hidden,emb_size)).to(device)
        h_t_1 = torch.randn((n_hidden,emb_size)).to(device)
        for x_t in X:
            
            i_t = torch.sigmoid(self.W_ii(x_t)+self.b_ii+self.W_hi(h_t_1) + self.b_hi)
            f_t = torch.sigmoid(self.W_if(x_t)+self.b_if+self.W_hf(h_t_1) + self.b_hf)
            g_t = torch.tanh(self.W_ig(x_t)+self.b_ig+self.W_hg(h_t_1) + self.b_hg)
            o_t = torch.sigmoid(self.W_io(x_t)+self.b_io+self.W_ho(h_t_1) + self.b_ho)
            ...
            # exit()
            # 第二层 输入是第一层输出
            i_t = torch.sigmoid(self.W_ii1(h_t)+self.b_ii1+self.W_hi1(h_t_1) + self.b_hi1)
            f_t = torch.sigmoid(self.W_if1(h_t)+self.b_if1+self.W_hf(h_t_1) + self.b_hf1)
            g_t = torch.tanh(self.W_ig1(h_t)+self.b_ig1+self.W_hg1(h_t_1) + self.b_hg1)
            o_t = torch.sigmoid(self.W_io1(h_t)+self.b_io1+self.W_ho1(h_t_1) + self.b_ho1)
        ...
        outputs = h_t # [batch_size, num_directions(=1) * n_hidden]
